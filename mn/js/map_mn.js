                ...
                if(F1.WorldBank.extractives[indicator].infosubtitle !== undefined && F1.WorldBank.extractives[indicator].infosubtitle !== null)
                  ...
                  try {
                    if(indicator!="Soum Boundaries"){
                        ...
                        self.map.setLayerInfoWindow(self.stylelayers[indicator].guid, {title: title + ": $["+ F1.WorldBank.extractives[indicator].styles.fill.selectedAttribute +"]", subtitle: infosub, tabs:infotabs});
                    }
                    else{
 self.map.setLayerInfoWindow(self.stylelayers[indicator].guid, {title: F1.WorldBank.extractives[indicator].title, subtitle: F1.WorldBank.extractives[indicator].subtitle, tabs:infotabs})
                    }
    ...
    minesPieChart: function(data)
        {
            var opts = {};
            var labels = ["аж ахуйн нэгжийн төтвөр","нэмүү өртөгийн татвар","лиценцийн хураамж","онцгой албан татвар","бүтээгдэхүүн хуваах","бусад төлбөр"];
...
//                    })
            ...
            pie_options = {"features":payments,
                "attributes": {"data":{"name": "нийт төлбөр","original_name": "total"},
                    "description":{"name": "төлбөрийн төрөл","original_name": "payment_type"},
                    "sort":{"name": "нийт төлбөр","original_name": "total"} } };
            ...
            jq("#sector_funding_total").html("нийт " + Math.round(total/100)/10 + " 10 сая төгрөг")
            
            var self = this;
            
            var table = '<table id="project-info"><thead><tr>';
            jq.each(["компаний нэр", "бүртгэлийн дугаар","байршил \\өгсөн бол\\", "хөрөнгө оруулалтын хэлэлцээр", "2010 улсын чанартай төлбөр", " 2010 орон нутгийн төлбөр", "2010 нийт төлбөр"], function(index,header) {
                    table += tmpl(table_templates.th, {id: index,header: header});
                    });
            table += "</tr></thead><tbody>"
    ...
    drawCharts: function() 
        {
            ...
            if(self.thematic_area == 'm4r'){
                ...
                jq('#active_projects_header').html("компани")
                jq('#activity_count').html(74);
                jq('#mapped_locations_header').html("байршилаар орсон")
                self.map.setMapStyle( {zoom: { offset: {x:15,y:90}}} )
                self.setExtractiveIndicator('Licenses','Production percent','Үйлдвэрлэл, сумын хувиар зураглал хийгдсэн компаниуд',true)
